...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onTotalGamesByUser = function onTotalGamesByUser(event) {
	  //  event.preventDefault()
	  var data = '{}';
	  api.totalGamesByUser(this.data).then(ui.totalGamesSuccess).catch(ui.totalGamesFailure);
	};
	...
	var onClickBoard = function onClickBoard(event) {
	  ...
	  if (currentPlayer === 'x') {
	    ...
	    api.updateGame(data).then(ui.updateGameSuccess);
	    if (over) {}
	    currentPlayer = 'o';
	  } else {
	    ...
	    numberMoves += 1;
	    api.updateGame(_data).then(ui.updateGameSuccess);
	    if (over) {}
	    // hide board
	    currentPlayer = 'x';
	  }
	...
	var isOver = function isOver(board, currentPlayer) {
	  gameBoard = board;
	...
	var onClearGame = function onClearGame(event) {
	  //  event.preventDefault()
	  currentPlayer = 'x';
	  gameWinner = false;
	  numberMoves = 0;
	...
	module.exports = {
	  ...
	  onTotalGamesByUser: onTotalGamesByUser
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var totalGamesByUser = function totalGamesByUser(data) {
	  return $.ajax({
	    url: config.apiOrigin + '/games/',
	    method: 'GET',
	    headers: {
	...
	module.exports = {
	  ...
	  totalGamesByUser: totalGamesByUser,
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var clearGameSuccess = function clearGameSuccess(data) {
	  $('#clearGame').modal('hide');
	};

	var clearGameFailure = function clearGameFailure(error) {};

	var totalGamesSuccess = function totalGamesSuccess(data) {
	  store.games = data.games;
	  var userGames = store.games.length;
	  var userGameArray = JSON.stringify(store.games);
	  $('h1').html('<p>User played ' + userGames + ' game(s).');
	  $('#totalGamesByUser').modal('hide');
	};

	var totalGamesFailure = function totalGamesFailure(error) {};
	...
	module.exports = {
	  ...
	  clearGameSuccess: clearGameSuccess,
	  clearGameFailure: clearGameFailure,
	  totalGamesSuccess: totalGamesSuccess,
	  totalGamesFailure: totalGamesFailure,
